...
void readFromStdin(std::vector<float> &bin_data)
{

	std::cin.seekg(0, std::ios::end);
	const unsigned int num_samples = std::cin.tellg() / sizeof(float);

	bin_data.resize(num_samples);
	std::cin.seekg(0, std::ios::beg);
	std::cin.read(reinterpret_cast<char*>(&bin_data[0]), num_samples*sizeof(float));
	//std::cin.close();
}
